...
for i=1:m,
   ...
   for j=1:n,
      ...
      if i0>=1 & i0<=M & j0>=1 & j0<=N & ...
             ...
             p1>=1 & p1<=M & q1>=1 & q1<=N,
             % Aviya and Snir
             % aviya is a new variable delete it for original code
             dm0=m0(i0:i1,j0:j1);
             aviya = 0;
             sz = size(dm0);
             dm0Snir=zeros(sz(1)+2*aviya,sz(2)+2*aviya) ;
             dm0Snir(aviya+1:sz(1)+aviya,aviya+1:sz(2)+aviya) = m0(i0:i1,j0:j1) ;
             ...
             sz = size(m1);
             m1Snir =zeros(sz(1)+2*aviya,sz(2)+2*aviya) ;
             m1Snir(aviya+1:sz(1)+aviya,aviya+1:sz(2)+aviya) = m1 ;
             dm1Snir = m1Snir(p0:p1+2*aviya,q0:q1+2*aviya) ;
             ...
             [du dv dc]=decorr2a(dm0Snir,dm1Snir);
